...
interface Props {
    attractions: { data: Attraction[] } & PaginationMeta;
    villages: Village[];
...
export default function AdminAttractionsIndex({
    attractions,
    villages,
    filters,
}: Props) {
    ...
    const { data, ...pagination } = attractions;

    return (
        <AdminLayout title="Wisata & Atraksi">
            <PageHeader
                title="Manajemen Wisata & Atraksi"
                breadcrumbs={[
            ...
            <DataTable
                ...
                editHref={(r) => `/admin/attractions/${r.id}/edit`}
                deleteRoute={(r) => `/admin/attractions/${r.id}`}
                deleteConfirmTitle="Hapus Wisata?"
                deleteConfirmMessage={(r) =>
                    `Yakin ingin menghapus wisata "${r.name}"?`
                }
                emptyMessage="Tidak ada wisata ditemukan."
            />
        </AdminLayout>
    );
}
