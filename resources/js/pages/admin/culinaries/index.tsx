...
interface Props {
    culinaries: { data: Culinary[] } & PaginationMeta;
    villages: Village[];
...
export default function AdminCulinariesIndex({
    culinaries,
    villages,
    filters,
}: Props) {
    ...
    const { data, ...pagination } = culinaries;

    return (
        <AdminLayout title="Kuliner & UMKM">
            <PageHeader
                title="Manajemen Kuliner & UMKM"
                breadcrumbs={[
            ...
            <DataTable
                ...
                editHref={(r) => `/admin/culinaries/${r.id}/edit`}
                deleteRoute={(r) => `/admin/culinaries/${r.id}`}
                deleteConfirmTitle="Hapus Kuliner?"
                deleteConfirmMessage={(r) =>
                    `Yakin ingin menghapus kuliner "${r.name}"?`
                }
                emptyMessage="Tidak ada kuliner ditemukan."
            />
        </AdminLayout>
    );
}
